...
public class FilmLikePutTest {
  ...
  @Test
  void addCorrectLike() throws Exception {
    performLikeFilm(film.getId(), userOne.getId())
        .andExpect(status().isOk())
        .andExpect(jsonPath(idPath).value(film.getId()))
        .andExpect(jsonPath(likeSetLengthPath).value(1))
        .andExpect(jsonPath(firstLikeIdPath).value(userOne.getId()));
  }

  @Test
  void addLikeTwiceIsIdempotent() throws Exception {
  ...
  @Test
  void addLikeFromMultipleUsers() throws Exception {
    performLikeFilm(film.getId(), userOne.getId()).andExpect(status().isOk());

    performLikeFilm(film.getId(), userTwo.getId())
        .andExpect(status().isOk())
        .andExpect(jsonPath(likeSetLengthPath).value(2));
  }

  // Тест ошибочных значений

  @Test
  void addLikeWithNonexistentFilm() throws Exception {
    performLikeFilm(9999L, userOne.getId()).andExpect(status().isNotFound());
  }

  @Test
  void addLikeWithNonexistentUser() throws Exception {
    performLikeFilm(film.getId(), 9999L).andExpect(status().isNotFound());
  }
}
